	...
	if (history[0]) {
		for (int i = 9; i > 0; i--) {	
			if (history[i-1]) {
				...
				history[i] = malloc(sizeof(history[i-1]) + 1);
				strcpy(history[i], history[i-1]);	
			}
		}
		free(history[0]);
	}
	history[0] = malloc(sizeof(str) + 1);
...
*/
			...
			default:
			{
			// If this was a dedicated program like zcat, we would use this
			// The shell is always going to call external commands as child processes so
			// Whats the point of a default? Maybe im missing something :///
				...
				lhp = strdup(cmd->cmd);
				lhp_argv = (char **) calloc(cmd->param_count + 2, sizeof(char *));
				lhp_argv[0] = lhp;
				while (NULL != param) {
					lhp_argv[param_index] = strdup(param->param);	
					param = param->next;
					param_index++;
				}
				execvp(lhp_argv[0], lhp_argv);
				perror("Parent could not exec program");
				_exit(EXIT_FAILURE);
				*/
			}
				wait(NULL);
...
/*		char ** ragged_array = NULL;
		...
		char * bptr = fgets(buffer, MAX_STR_LEN, iFile);
		while (bptr != NULL) {
			ragged_array = realloc(ragged_array, (numberOfElements + 1) * sizeof(char *));
			ragged_array[numberOfElements++] = strdup(buffer);
		
			bptr = fgets(buffer, MAX_STR_LEN, iFile);
